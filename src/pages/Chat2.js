...
// calling it quits now, kosy i'm tired
...
const Chat2 = ({videoChat = true}) => {
  ...
  const Initialize = (_token) =>{
    const _chat = new ChatApi2(
        ...
        {
            onParticipantAdded: setUser,
            onMediaLoaded: OnMediaLoaded
        }
   
      );
  
  _chat.initialize(_token).then(console.log)
  setChat(chat)
  }
  useEffect(() => {
    av();
...
<input id="lol" type="text"  onChange={(e) => {
